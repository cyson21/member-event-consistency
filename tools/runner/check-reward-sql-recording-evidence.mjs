...
const outputDir = mkdtempSync(join(tmpdir(), 'mec-reward-sql-recording-evidence-'));
...
function routeEntry(summary, strategy) {
  const entry = summary.routeEntries.find((candidate) => (
    candidate.scenario === 'FIRST_LOGIN_REWARD' && candidate.strategy === strategy
  ));
  if (!entry) {
    throw new Error(`Missing FIRST_LOGIN_REWARD route entry for ${strategy}`);
  }
  return entry;
}
...
try {
  ...
  expectSqlEvidence(summary, 'NAIVE', 'duplicate-prone attempt insert -> fake follow-up outbox rows');
  expectSqlEvidence(summary, 'DB_GUARD', 'unique reward issue insert -> fake follow-up outbox rows');
  expectSqlEvidence(summary, 'REDIS_LOCK_DB_GUARD', 'unique reward issue insert -> fake follow-up outbox rows');

  const redis = routeEntry(summary, 'REDIS_LOCK_DB_GUARD');
  if (redis.evidence !== 'issued=1, duplicate=0') {
    throw new Error(`Redis reward route must keep duplicate prevention evidence: ${JSON.stringify(redis)}`);
  }

  console.log(JSON.stringify({
    status: 'passed',
    checkedRewardStrategies: 3,
